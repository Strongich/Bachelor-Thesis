...
from Ferret.ImplicitEu import ImplicitEu
...
class ImplicitAmBer(ImplicitEu):
    ...
    def _setup_boundary_conditions_(self):
        ...
        ]  # advanced indexing - a copy not a view
        ...
        if self.is_call:
            ...
            for j in reversed(self.jValues):
                ...
                for i in reversed(range(1, self.M)):
                    if i == self.M - 1:
                        self.grid[i, j] = (
                            self.grid[i, j + 1] / self.coeffs[i - 1, i - 1]
                        )
                    else:
                        self.grid[i, j] = (
                            self.grid[i, j + 1]
                            - self.grid[i + 1, j] * self.coeffs[i - 1, i]
                        ) / self.coeffs[i - 1, i - 1]
            ...
            for i in reversed(range(1, self.M - 1)):
                ...
                self.coeffs[i - 1, :] -= (
                    self.coeffs[i, :] * self.coeffs[i - 1, i] / self.coeffs[i, i]
                )
            ...
            for j in reversed(self.jValues):
                ...
                for i in range(1, self.M):
                    if i == 1:
                        ...
                        self.grid[i, j] = (
                            self.grid[i, j + 1]
                            - self.grid[i - 1, j] * self.coeffs[i - 1, i - 2]
                        ) / self.coeffs[i - 1, i - 1]
                    if self.grid[i, j] < self.IV[i - 1]:
                        self.grid[i, j] = self.IV[i - 1]
                self.grid[0, j] = 2 * self.grid[1, j] - self.grid[2, j]
